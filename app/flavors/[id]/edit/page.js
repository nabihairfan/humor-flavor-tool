...
export default function EditFlavor() {
  ...
  useEffect(() => {
    supabase.from("humor_flavors").select("*").eq("id", id).single().then(({ data }) => {
      if (data) setForm({ slug: data.slug, description: data.description || "" })
    })
  }, [id])
  ...
  async function handleSubmit() {
    ...
    setSaving(true)
    const { error } = await supabase.from("humor_flavors").update(form).eq("id", id)
  ...
  return (
    <main className="min-h-screen bg-gray-950 p-8">
      ...
      <h1 className="text-3xl font-black text-white mb-8">✏️ Edit Humor Flavor</h1>
